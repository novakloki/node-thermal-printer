module.exports = {
  ...
  CTL_VT: Buffer.from([0x1b, 0x64, 0x04]), // Vertical tab `ESC d`

  // Printer hardware
  HW_INIT: Buffer.from([0x1b, 0x40]), // Clear data in buffer and reset modes `ESC @`
  ...
  UPSIDE_DOWN_ON: Buffer.from([0x1b, 0x7b, 0x01]), // Upside down printing ON (rotated 180 degrees). `ESC {`
  UPSIDE_DOWN_OFF: Buffer.from([0x1b, 0x7b, 0x00]), // Upside down printing OFF (default). `ESC {`
  ...
  QRCODE_PRINT_SELECT: Buffer.from([0x1d, 0x5a, 0x02, ]), // Select QR code
